...
export function Callout({ children, title, tone = "note", icon, style, ...rest }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)", textAlign: "start" }}>
        {title ? (
          <strong style={{ font: "var(--weight-semibold) var(--text-sm) / 1.3 var(--font-display)", color: t.fg }}>
            {title}
          </strong>
        ) : null}
